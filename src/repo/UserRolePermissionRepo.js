  ...
  try {
    ...
    for (const userRole of userRoles) {
      const permissions = await new Promise((resolve, reject) => {
        PermissionRepositoryRef.fetchPermissionsByRoleId(
          userRole.roleId,
          (permissions) => {
...
async function processRoleDefaultPrivileges(req) {
  ...
  if (role != null) {
    ...
    console.log("specified role to be added not available");
  }
          ...
          if (permissionIds != null) {
            permissionIds.forEach((permissionId) => {
              setDefaultPrivileges({
                userId: req.userId,
                permissionId: permissionId.dataValues.permissionsId,
...
async function isExistUserRolePermissionsThen(userRoleId, permissionId) {
  const query = `
      SELECT UserRolesPermissions.status, Permissions.permissionName as permissionName
      FROM UserRolesPermissions
      INNER JOIN Permissions ON UserRolesPermissions.permissionId = Permissions.id
      WHERE UserRolesPermissions.userRoleId = :userRoleId and 
      UserRolesPermissions.permissionId = :permissionId
    `;
...
async function createUpdateUserRolesPermission(req) {
  UserRolesPermissionRepository.findOne({
    where: { userRoleId: req.userRoleId, permissionId: req.permissionId },
  })
    .then((userRolePermission) => {
      if (userRolePermission) {
        return userRolePermission.update({ status: req.status });
      } else {
        console.log("Record not found, creating...")
        UserRolesPermissionRepository.create(req);
      }
    })
    .then((userRolePermission) => {
...
async function deleteUserRolePermissionsRecord(userRoleId) {
  try {
...
async function createUserRolesPermission(req, callback) {
  const result = await UserRolesPermissionRepository.create(req);
  callback(result);
}
...
module.exports = {
  fetchPermissions,
  initiatePermissionsForUser,
  setDefaultPrivileges,
  processRoleDefaultPrivileges,
  isExistPriviledge,
  getUserRolePermissions,
  getUserRolePermissionsThen,
  isExistUserRolePermissionsThen,
  createUserRolesPermission,
  createUpdateUserRolesPermission,
  deleteUserRolePermissionsRecord
};
